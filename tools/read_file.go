...
// ReadFile อ่านไฟล์ รองรับ offset/limit เพื่อไม่ให้ context พองเวลาเจอไฟล์ใหญ่
type ReadFile struct{}

func (ReadFile) Name() string { return "read_file" }

func (ReadFile) Definition() anthropic.ToolParam {
	return anthropic.ToolParam{
		Name:        "read_file",
		Description: anthropic.String("อ่านเนื้อหาไฟล์ตาม path ที่ระบุ คืนผลพร้อมเลขบรรทัด ใช้ offset/limit เมื่อไฟล์ใหญ่เพื่ออ่านทีละช่วง"),
...
func (ReadFile) Execute(input json.RawMessage) (string, error) {
	...
	sc.Buffer(make([]byte, 0, 64*1024), 1024*1024) // รองรับบรรทัดยาว
	lineNo := 0
	printed := 0
	for sc.Scan() {
		lineNo++
